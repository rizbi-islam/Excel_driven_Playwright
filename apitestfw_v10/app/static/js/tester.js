...
const Tester = (() => {
  ...
  function showRespTab(name, el) {
    document.querySelectorAll('[onclick*="showRespTab"]').forEach(b => b.style.background = '');
  ...
  function _clearResp() {
    ...
    const bodyEl = document.getElementById('resp-body');
    if (bodyEl) { bodyEl.style.color = 'var(--text3)'; bodyEl.textContent = 'Send a request to see the response here.'; }
    const hdrsEl = document.getElementById('resp-headers-view');
    if (hdrsEl) { hdrsEl.textContent = ''; hdrsEl.style.display = 'none'; }
    document.getElementById('resp-body').style.display = '';
  }
