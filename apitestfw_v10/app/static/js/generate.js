...
const Generate = (() => {
  ...
  function _generateCases(ep, analysis, type, count) {
    ...
    const base = {
      ...
      // Default tests script per type
      tests_script: _defaultTests(type, probeStatus),
      pre_request_script: '',
    };
  ...
  function _defaultTests(type, expectedStatus) {
    const lines = [`pm.test("Status ${expectedStatus}", lambda: pm.expect(pm.response.code).to_equal(${expectedStatus}))`];
    if (type === 'regression') {
      lines.push(`pm.test("Response time < 5000ms", lambda: pm.expect(pm.response.responseTime).to_be_below(5000))`);
    }
    if (type === 'smoke') {
      lines.push(`pm.test("Response time < 3000ms", lambda: pm.expect(pm.response.responseTime).to_be_below(3000))`);
    }
    if (type === 'security') {
      lines.push(`pm.test("No server error", lambda: pm.expect(pm.response.code).to_be_below(500))`);
    }
    return lines.join('\n');
  }
